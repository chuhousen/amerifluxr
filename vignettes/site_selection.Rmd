...
of metadata and data. See 
[README](https://chuhousen.github.io/amerifluxr/index.html#use) 
for data download examples. A companion vignette for 
...
pander::pandoc.table(sites_dt[!is.na(DATA_START), .N, by = .(IGBP, DATA_POLICY)])
...
In addition, users can use amf_list_data() to 
query the availability of specific variables 
in the data (i.e., flux/met data, so-called BASE
data product). The amf_list_data() provides a quick
summary of variable availability (per site/year)
...
# return a list of sites with multiple WS variables
pander::pandoc.table(crop_ls4_aval <- data_aval[data_aval$SITE_ID %in% crop_ls4$SITE_ID &
                                                  data_aval$BASENAME == "WS",
                                                .(SITE_ID, VARIABLE, Y2015, Y2016, Y2017, Y2018)])
...
See [README]
(https://chuhousen.github.io/amerifluxr/index.html#use)
for data download examples.
